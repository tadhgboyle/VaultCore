...
public class DiscordCommand implements CommandExecutor {
	...
	private String getToken(UUID uuid, Player player) throws SQLException {

		java.sql.Statement stmt = VaultCore.getInstance().connection.createStatement();
		ResultSet rs = stmt.executeQuery("SELECT discord_token FROM players WHERE uuid='" + uuid + "'");

		if (rs.next()) {
			String token = rs.getString("discord_token");
			if (token != null) {
				return token;
			}
		}
		...
		VaultCore.getInstance().connection.createStatement()
				.executeUpdate("UPDATE players SET discord_token='" + new_token + "' WHERE uuid='" + uuid + "'");
		return new_token;
	}
}
